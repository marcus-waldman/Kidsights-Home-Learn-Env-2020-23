...
####
     # Wrangle race variable
      # RAPID
      RaceGroup_xwalk = data.frame(RaceGroup = sjlabelled::get_labels(rapid23$RaceGroup), 
                                   race4_char = c("Other/Multi-racial, non-Hispanic", 
                                                  ...
                                                  "Hispanic"))
      rapid23 = rapid23 %>% 
        dplyr::mutate(race4_char = plyr::mapvalues(sjlabelled::to_character(RaceGroup), from = RaceGroup_xwalk$RaceGroup, to = RaceGroup_xwalk$race4_char ))
      
      rapid23 = rapid23 %>% 
        dplyr::mutate(black = startsWith(race4_char,"Black") %>% as.integer(),
                        hisp = startsWith(race4_char, "Hispanic") %>% as.integer(),
                        other = startsWith(race4_char, "Other") %>% as.integer(),
                        white = startsWith(race4_char,"White") %>% as.integer())
      # NE
      dat = dat %>% dplyr::mutate(black = startsWith(race4_char,"Black") %>% as.integer(),
                                  hisp = startsWith(race4_char, "Hispanic") %>% as.integer(),
                                  other = startsWith(race4_char, "Other") %>% as.integer(),
                                  white = startsWith(race4_char,"White") %>% as.integer())
  ...
  # Wrangle Education 
      # RAPID
      rapid23$DEMO.014[dat$DEMO.014==9] = NA
      rapid23 = rapid23 %>% 
        dplyr::mutate(
          nohs = as.integer(DEMO.014<3), 
          hs = as.integer(DEMO.014==3), 
          somecoll = as.integer(DEMO.014>3 & DEMO.014<6), 
          bs = as.integer(DEMO.014==6), 
          above_bs = as.integer(DEMO.014>6 & DEMO.014<9)
        )
      
      # NE
      foo = dat22 %>% 
        dplyr::select(mrwid, CQR004) %>% 
        dplyr::mutate(across(everything(), zap_attributes)) %>% 
        dplyr::bind_rows(
          dat20 %>% dplyr::select(mrwid, CQR004) %>% rename(CQR004_20 = CQR004) 
        )
      dat = dat %>% dplyr::left_join(foo, by = "mrwid")
      dat$CQR004_20 = as.character(dat$CQR004_20)
      dat = dat %>% dplyr::mutate(
        nohs = as.integer(a1_edu4==0),
        hs = as.integer(a1_edu4==1),
        somecoll = as.integer(a1_edu4==2),
        bs = ifelse(mrwid<0,CQR004==7,startsWith(CQR004_20, "Bachelor")) %>% as.integer(), 
        above_bs = ifelse(mrwid<0, CQR004>7, startsWith(CQR004_20,"Master") |  startsWith(CQR004_20,"Doctorate")) %>% as.integer()
      )
      ...
      # RAPID
      rapid23 = rapid23 %>% 
        dplyr::mutate(inc99=get_cpi99(2023)*Annual2023Income) %>% 
        dplyr::mutate(PHQ2a = zap_attributes(PHQ.002.a), 
                      PHQ2b = zap_attributes(PHQ.002.b), 
                      GAD2a = zap_attributes(GAD2.002.a), 
                      GAD2b = zap_attributes(GAD2.002.b)
        ) %>% 
        dplyr::mutate(depanx_tot = PHQ2a + PHQ2b + GAD2a + GAD2b)
      
      
      # NE #note that 2022 needs to be zer coded
      dat = dat %>% 
        dplyr::mutate(
          PHQ2a = ifelse(mrwid<0,PHQ2a-1, PHQ2a), 
          PHQ2b = ifelse(mrwid<0, PHQ2b-1, PHQ2b), 
          GAD2a = ifelse(mrwid<0, GAD2a-1,GAD2a), 
          GAD2b = ifelse(mrwid<0, GAD2b-1, GAD2b),
          depanx_tot = PHQ2a+PHQ2b +GAD2a + GAD2b, 
          )
      
      
#### !! This is where I stopped 05/30/2025 ####
      
      ### combine and load in the scores
      
      criterion_all = dat %>% 
        dplyr::filter(mrwid < 99000) %>% 
        dplyr::select(mrwid,depanx_tot, inc99, black:above_bs, PHQ2a,PHQ2b,GAD2a,GAD2b) %>% 
        dplyr::bind_rows(rapid23 %>% dplyr::select(mrwid, depanx_tot, inc99, black:above_bs, PHQ2a:GAD2b)) %>% 
        dplyr::left_join(fscores %>% dplyr::rename(kidsight = F1), by = "mrwid") %>% 
        dplyr::filter(!is.na(kidsight))
      
      
      criterion_all = criterion_all %>% 
        dplyr::mutate(source = ifelse(mrwid >= 99000, "RAPID","Nebraska"), 
                      educ = nohs*0 + hs*1 + somecoll*2 + bs*3 + above_bs*4, 
                      educ = educ %>% ordered(level = 0:4, labels = c("No High School Diploma", "High School Diploma", "Some College (including AA/AS)", "Bachelor's degree", "Master's, Doctorate, or Professional Degree")), 
                      depanx_tot = (depanx_tot-mean(depanx_tot,na.rm=T))/sd(depanx_tot,na.rm=T), 
                      log_inc = log10(inc99+1000), 
                      s_log_inc = (log_inc - mean(log_inc,na.rm =T))/sd(log_inc,na.rm = T))
      
      
       criterion_mice = criterion_all %>% 
         dplyr::select(source, 
                       years,
                       depanx_tot,
                       black:other,
                       ...
                       above_bs,
                       s_log_inc) %>% 
        mice::mice(m=10, method = "cart")
      
       
       criterion_implist = lapply(1:10, function(m){
         complete(criterion_mice,m) %>% 
           dplyr::bind_cols(criterion_all %>% dplyr::select(mrwid)) %>% 
           dplyr::relocate(mrwid) %>% 
           dplyr::left_join(itemdat %>% dplyr::select(mrwid,AA10:DD9), by = "mrwid")}
         )
